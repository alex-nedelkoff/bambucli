...
A deep internal reference for the **design and data flow** of the app — read it
end-to-end to feel grounded on every component, then demo, debug, or talk through
any layer with confidence.

> **Historical note:** this doc was written during the original **macOS**
> development, so some file paths, install scripts (`install.sh`, launchd), and
> Tailscale notes below reflect that era rather than the current **Windows**
> kiosk. The architecture and data-flow descriptions are still accurate; for
> current platform/operations specifics, the authoritative doc is
> [OPERATIONS.md](OPERATIONS.md), and the current repo layout is in the top-level
> `README.md`.

Companion doc: [glossary.md](glossary.md) — every term here with a plain +
technical definition. When something uses a term that doesn't ring a bell, the
glossary is the lookup. Privacy/MFIPPA obligations live in
