...
doc. For code architecture see [architecture.md](architecture.md).
...
> internal design notes in `architecture.md` predate the Windows move and still
> show macOS paths — treat this file as authoritative for operations.)
...
powershell -ExecutionPolicy Bypass -File deploy\register-ollama-service.ps1
```
...
| Restart "didn't work" (same PID on :8000) | You used `Stop-ScheduledTask`; use `deploy\register-task.ps1` elevated instead (§3). |
...
- This `mcleans` branch is a clean, history-free production snapshot; ongoing
  development happens on other branches.
