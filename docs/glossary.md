...
- **Plain:** The thermal receipt printer USB-attached to the kiosk.
- **Tech:** The production unit is a generic 80 mm ESC/POS receipt printer
  that enumerates as "USB Receipt Printer", vendor ID `0x0FE6`, product ID
  `0x811E`, driven over WinUSB via python-escpos (see OPERATIONS.md §6).
  80 mm paper width (~42 chars at default font), autocutter, ESC/POS protocol.
- **Legacy:** The original development hardware was an Epson TM-T88V
  (VID `0x04B8` / PID `0x0202`); it has since been swapped for the generic
  clone above. You may still see "TM-T88V" referenced in older notes.
